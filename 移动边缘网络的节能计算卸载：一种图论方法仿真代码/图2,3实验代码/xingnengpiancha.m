...
for N=10:10:150
    ...
    perkm(1,kk)=0;
    permadf(1,kk)=(resmadf(1,kk)-reskm(1,kk))/reskm(1,kk).*100;
    permsdf(1,kk)=(resmsdf(1,kk)-reskm(1,kk))/reskm(1,kk).*100;
    permdf(1,kk)=(resmdf(1,kk)-reskm(1,kk))/reskm(1,kk).*100;
...
%设置横纵坐标的标题
title('与KM算法比较的性能差异'); %标题
L1=xlabel('UE的数量'); %x轴标题
L2=ylabel('相对偏差（%）'); %y轴标题
legend('KM','MADF-GS','MSDF-GS','MDF-GS','Location','NorthEast ') %为图片添加图例
xlim([10 150])
ylim([0 30])
